...
Your patches + the `client-manifest` are then stored somewhere that is HTTPS accessible, preferably on some kind of global CDN (e.g. Cloudflare, S3 etc).
...
During gameplay the patches are used in a random-access mode we call **livepatching** and the full patched UO asset is never written to disk to save players disk-space.
...
### Target directory
This is your shards files. You can use your UO directory itself if you want as only the files you list will be patched.

### Output directory
This is where the `.patch` files and your `client-manifest.json` will be written. This is the directory you need to upload
...
| **region**        | "US" or "EU" or "OCE" or "ASIA" or "BR" | "US"                                                              | If we're missing some please raise an issue.                                                                                                                |
...
The CLI will take some, when patching is complete and your **output** folder should contain `.patch` files and your `client-manifest.json`.
You can now take these files and upload them to your hosting/CDN where they should be accessible at the URL you set for `cdnBase`.
...
Double-check your file list, it's likely you have a UOP and MUL version of the same assets, e.g. `art.mul` and `artLegacyMUL.uop`. Use only one.
