...
abstract class SHAManagedImplementationBase
{
...
internal sealed class SHA256ManagedImplementation : SHAManagedImplementationBase
{
  ...
  context.
  */
  public unsafe void HashCore(Span<byte> partIn, int ibStart, int cbSize)
  {
    int bufferLen;
    ...
    fixed (uint* stateSHA256 = _stateSHA256)
    {
      fixed (byte* buffer = _buffer)
      {
        fixed (uint* expandedBuffer = _W)
        {
          ...
          {
            partIn.Slice(partInBase, 64 - bufferLen).CopyTo(_buffer.AsSpan(bufferLen));
          ...
          while (partInLen >= 64)
          {
            partIn.Slice(partInBase, 64).CopyTo(_buffer);
          ...
          if (partInLen > 0)
          {
            partIn.Slice(partInBase, partInLen).CopyTo(_buffer.AsSpan().Slice(bufferLen));
